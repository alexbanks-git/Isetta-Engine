...
void AudioModule::StartUp() {
  ...
  FMOD::Memory_Initialize(MemoryManager::AllocOnStack(CONFIG_VAL(audioConfig.memorySize)),
...
void AudioModule::CheckStatus(const FMOD_RESULT status) {
  if (status != FMOD_OK) {
    ...
    //throw std::exception{Util::StrFormat("AudioSource::CheckStatus => %s",FMOD_ErrorString(status))};
  }
}
...
void AudioModule::LoadClip(AudioClip* const clip) const {
  ...
  CheckStatus(fmodSystem->createSound(filePath.c_str(), FMOD_LOWMEM, nullptr,&clip->fmodSound));
}
...
void AudioModule::Play(AudioSource* const source) const {
  ...
  if (source->GetProperty(Property::IS_3D))
    source->fmodChannel->set3DMinMaxDistance(source->minMaxDistance.x,source->minMaxDistance.y);
}
}  // namespace Isetta
