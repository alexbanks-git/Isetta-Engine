...
void Console::OnEnable() {
  //Logger::outputCallback =std::bind(&Console::AddLog, this, std::placeholders::_1);
  ...
  GUI::Window(
      ...
      [&]() {
        ...
        for (int verbosity = static_cast<int>(Debug::Verbosity::All) - 1;
          ...
          //displayVerbosity[verbosity - 1] = Logger::verbosityMask.test(verbosity - 1);
          std::string label = Debug::ToString(static_cast<Debug::Verbosity>(verbosity));
       ...
       // }
        GUI::ButtonDropDown(rect, "Channel", Math::Vector2{30, 20}, [&]() {
          ...
          for (int channel = 0; channel < static_cast<int>(Debug::Channel::All);
               ++channel) {
            //displayChannel[channel] = Logger::channelMask.test(channel);
            std::string label = Debug::ToString(static_cast<Debug::Channel>(channel));
            GUI::Toggle(rect, label,&displayChannel[allChannel]);
