...
static std::unordered_map<std::string_view,Action<Console* const, std::string_view>>userCmds;
int historyPos = -1;
bool scrollToBottom;
...
//Console(std::string title, bool isOpen, const GUI::WindowStyle& style,const Color textColors[(int)Debug::Verbosity::All]);
~Console() override;
...
static void AddCommand(const std::string_view& cmd,const Action<Console* const, std::string_view>& callback);
