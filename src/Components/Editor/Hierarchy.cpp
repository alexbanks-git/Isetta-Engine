...
Hierarchy::Hierarchy(std::string title, bool isOpen, Inspector* inspector): title{title}, isOpen{isOpen}, inspector{inspector} {}
  ...
  GUI::Window(
      ...
      [&]() {
        ...
        Action<Transform*> action = [&](Transform* t) {
          ...
          if (GUI::Button(RectTransform{Math::Rect{left + offset, height,buttonWidth - offset, buttonHeight}},t->GetName())) {
            target = t;
          }
          GUI::PopID();
