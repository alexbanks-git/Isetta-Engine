...
Inspector::Inspector(std::string title, const bool isOpen, bool isStatic, Transform *const target): target{target}, title{title}, isOpen{isOpen} {
...
void Inspector::GuiUpdate() {
  ...
  GUI::Window(
      rectTransform, title,
      [&]() {
        std::string parentName = target->GetParent() ? target->GetParent()->GetName() : "null";

        float height = 5;
        float padding = 15;
        RectTransform rect = RectTransform{Math::Rect{padding, height, 300, 100}};
        ...
        GUI::Text(rect,"World Rotation " + target->GetWorldEulerAngles().ToString());
        rect.rect.y += 20;
        ...
        if (GUI::Button(RectTransform{Math::Rect{0, rect.rect.y, 300, 30},GUI::Pivot::Top, GUI::Pivot::Top},"Reset")) {
