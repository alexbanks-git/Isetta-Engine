...
{
  channelStream.flush();
  Filesystem::Instance().WriteAsync(channelFileName, channelStream.str());//buulshit
  channelStream.str("");
  channelStream.clear();
  channelStream.seekp(0, std::ios::beg);

  engineStream.flush();
  Filesystem::Instance().WriteAsync(engineFileName, engineStream.str());//bullshit
  engineStream.str("");
  engineStream.clear();
                         ...
                         const char *buffer)
{
  *stream << buffer;
  ...
  {
    stream->flush();
    Filesystem::Instance().WriteAsync(fileName, stream->str());//bullshit
    stream->str("");
    stream->clear();
