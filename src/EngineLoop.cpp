...
//#endif
  ...
  std::cout << "PRE_LEVELMANAGER_INSTANCING: loading_level_name: " << CONFIG_VAL(levelConfig.startLevel) << std::endl;
  LevelManager::Instance().LoadLevel(CONFIG_VAL(levelConfig.startLevel));
  std::cout << "POST_LEVELMANAGER_INSTANCING: loading_level_name: " << CONFIG_VAL(levelConfig.startLevel) << std::endl;
