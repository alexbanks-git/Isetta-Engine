...
// BUTTON
bool GUI::Button(Func<bool> button,
...
bool GUI::Button(const RectTransform& transform,
		         const std::string& label,
                 //const ButtonStyle& style,
				 bool repeating) {
  Func<bool> btn;
  btn = std::bind(ImGui::Button, label.c_str(), (ImVec2)transform.rect.Size());
  return Button(btn, transform, repeating);
}
bool GUI::Button(const RectTransform& transform,
		         const std::string& label,
				 ...
				 //const ButtonStyle& style,
                 bool repeating) {
  if (Button(transform, label, repeating)) {
    callback();
    return true;
  }
  return false;
...
void GUI::Toggle(const RectTransform& transform,
		         const std::string& label,
                 bool* value, const Action<>& callback
                 ) {
  bool lastFrame = value;
  Toggle(transform, label, value);
  if (*value != lastFrame) {
    callback();
  }
}
...
void GUI::Text(const RectTransform& transform, const std::string_view format) {
  ...
  //ImGui::PushStyleColor(ImGuiCol_Text,!style.isDisabled? (ImVec4)style.text: context->Style.Colors[ImGuiCol_TextDisabled]);
  //bool need_wrap = style.isWrapped &&(context->CurrentWindow->DC.TextWrapPos <0.0f);  // Keep existing wrap position is one ia already set
...
// WINDOWS
bool GUI::Window(const RectTransform& transform,
...
bool GUI::MenuItem(const std::string& label, const std::string& shortcut,bool selected, bool enabled) {
	return ImGui::MenuItem(label.c_str(), shortcut.c_str(), selected, enabled);
}

void GUI::MenuItem(const std::string& label, const std::string& shortcut,const Action<>& callback, bool enabled) {
