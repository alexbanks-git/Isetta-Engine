...
void NetworkTransform::Update() {
  if (posInterpolation < 1 || rotInterpolation < 1 || scaleInterpolation < 1) {
    ...
    // Translation
    posInterpolation =
        std::min(posInterpolation + Time::GetDeltaTime() / netIdLerp, 1.);
    t->SetLocalPos(Math::Vector3::Lerp(prevPos, targetPos, posInterpolation));
    // Rotation
    rotInterpolation =
    		std::min(rotInterpolation + Time::GetDeltaTime() / netIdLerp, 1.);
    t->SetLocalRot(
        Math::Quaternion::Slerp(prevRot, targetRot, rotInterpolation));
    // Scale
    scaleInterpolation =
    		std::min(scaleInterpolation + Time::GetDeltaTime() / netIdLerp, 1.);
    t->SetLocalScale(
