...
void NetworkingModule::StartUp() {
  ...
  if (!InitializeYojimbo()) {
    //throw std::exception("NetworkingModule::StartUp => Could not initialize yojimbo.");
  }
...
void NetworkingModule::AddClientToServerMessage(yojimbo::Message* message) const {
  if (!IsClientRunning()) {
	 LOG_ERROR(Debug::Channel::Networking,"Cannot send message from client cause client is not running");
	    return;
  }
...
void NetworkingModule::AddServerToClientMessage(int clientIdx,yojimbo::Message* message) {
  if (!IsServerRunning()) {
	  LOG_ERROR(Debug::Channel::Networking,"Cannot send message from server cause server is not running");
	    return;
  }
      ...
      if (state == static_cast<int>(NetworkManager::ClientState::Connected)) {
        NetworkManager::Instance().SendMessageFromClient<ClientConnectedMessage>(
            [](ClientConnectedMessage* message) {
              strcpy(message->ip, SystemInfo::GetIpAddressWithPrefix(CONFIG_VAL(networkConfig.ipPrefix)).c_str());
              strcpy(message->machineName, SystemInfo::GetMachineName().c_str());
            });
...
void NetworkingModule::Disconnect() const {
  if (client->IsConnected()) {
    ...
    // Unregister callbacks
       NetworkManager::Instance().UnregisterClientCallback<LoadLevelMessage>(loadLevelCallbackHandle);
  } else if (!client->IsConnecting()) {
    return;
  } else {
   // throw std::exception("NetworkingModule::Disconnect => Cannot disconnect the client if it is ""not already connected.");
  }
}
...
void NetworkingModule::CloseServer() {
	 if (!IsServerRunning()) {
	    //throw std::exception("NetworkingModule::CloseServer() Cannot close the server if it is not ""running.");
	  }
	  ...
	  NetworkManager::Instance().UnregisterClientCallback<ClientConnectedMessage>(clientConnectedCallbackHandle);
}
